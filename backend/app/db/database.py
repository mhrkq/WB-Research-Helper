...
async def enable_pgvector(conn):
    try:
        ...
        logger.warning(f"pgvector extension already exists or cannot be created: {e}")
        
async def create_vector_index(conn):
    """
    Create HNSW index for document_chunks.embedding if it does not exist.
    """
    try:
...
async def init_db() -> None:
    ...
    try:
        async with engine.begin() as conn:
            await enable_pgvector(conn)     # add vector extension
            await conn.run_sync(Base.metadata.create_all)
            await create_vector_index(conn) # create vector index
